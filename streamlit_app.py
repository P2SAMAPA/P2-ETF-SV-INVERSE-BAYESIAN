...
st.set_page_config(page_title="Stochastic Volatility Inverse Problem (Bayesian Inversion)", layout="wide")
...
st.markdown('<h1 style="text-align: center;">📊 Stochastic Volatility Inverse Problem (Bayesian Inversion)</h1>', unsafe_allow_html=True)
st.markdown('<p style="text-align: center;">Latent volatility estimation | Laplace approximation | Posterior variance as uncertainty signal</p>', unsafe_allow_html=True)

st.sidebar.markdown("## 🧮 Bayesian Inversion")
...
st.sidebar.markdown(f"**Prior volatility:** {config.PRIOR_VOL} | **Observation noise:** {config.OBS_NOISE}")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'sv_inverse_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>Uncertainty score: {etf['inv_score_norm']:.3f}</p>
...
with tab1:
    st.header("📊 Top ETFs by Posterior Volatility Uncertainty (Auto Best Window)")
    with st.expander("📖 Interpretation", expanded=False):
        st.markdown("""
        - **Bayesian inversion** treats latent volatility as an unknown field.
        - We assume returns = vol * ε, with prior on log(vol) ~ N(0, σ_prior²).
        - Laplace approximation yields posterior variance of log‑volatility.
        - **Score = posterior variance** – high = uncertain volatility regime → potential alpha/regime shift.
        - Prior variance is adapted using macro variables (higher macro → higher prior uncertainty).
        """)
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the uncertainty scores.")
    for universe_name, uni_data in data["universes"].items():
        if not uni_data or not uni_data.get("all_windows"):
            st.warning(f"No window data for {universe_name}")
            continue
...
st.sidebar.markdown("---")
st.sidebar.caption("Stochastic Volatility Inverse Problem | Bayesian inversion for latent volatility")
