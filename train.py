...
from sv_inverse import bayesian_inversion_score
...
def run_for_window(returns, macro_df, window_days):
    ...
    for ticker in ret_window.columns:
        s = bayesian_inversion_score(
            ret_window[ticker].values,
            macro_window,
            prior_vol=config.PRIOR_VOL,
    ...
    top_etfs = [{"ticker": t, "inv_score_norm": s, "raw_score": raw_scores[t]} for t, s in sorted_norm[:config.TOP_N]]
    return {
        "window": window_days,
        "top_etfs": top_etfs,
...
def main():
    ...
    out_file = f"output/sv_inverse_{timestamp}.json"
